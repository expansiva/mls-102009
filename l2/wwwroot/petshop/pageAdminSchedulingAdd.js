/// <mls shortName="pageAdminSchedulingAdd" project="102009" enhancement="_blank" folder="wwwroot/petshop" />
// virtual:lit
var t = globalThis;
...
function subscribe2(keyOrKeys, component) {
  return globalState2.globalStateManagment.subscribe(keyOrKeys, component);
}
function unsubscribe2(keyOrKeys, component) {
  return globalState2.globalStateManagment.unsubscribe(keyOrKeys, component);
}
...
// virtual:/_102009_petshop/pageAdminSchedulingAdd
...
var PageAdminSchedulingAdd102009 = class PageAdminSchedulingAdd1020092 extends CollabPageElement {
  constructor() {
    super();
    if (this.loadStyle)
      this.loadStyle(`petshop--page-admin-scheduling-add-102009 {
  display: flex;
}
petshop--page-admin-scheduling-add-102009 main {
  height: 100vh;
  overflow: auto;
  width: 100%;
}
@media (max-width: 768px) {
  petshop--page-admin-scheduling-add-102009 {
...
`);
  ...
  initPage() {
    setState2("ui.petshop.admin.menu.selected", "scheduling");
  }
};
PageAdminSchedulingAdd102009 = __decorate3([
  e2("petshop--page-admin-scheduling-add-102009")
], PageAdminSchedulingAdd102009);
...
// virtual:/_102009_petshop/organismAdminScheduling
...
var organismAdminScheduling = class organismAdminScheduling2 extends IcaOrganismBase {
  constructor() {
    super(...arguments);
    if (this.loadStyle)
      this.loadStyle(`petshop--organism-admin-scheduling-102009 .scheduling-container{padding:var(--space-24);background-color:var(--bg-primary-color)}petshop--organism-admin-scheduling-102009 .user-list{list-style:none;padding:0;margin-bottom:var(--space-16);margin-top:var(--space-16)}petshop--organism-admin-scheduling-102009 .user-item{display:flex;align-items:center;gap:12px;background:#fff;border:1px solid #e5e7eb;border-radius:8px;padding:10px 14px;cursor:pointer;transition:all .2s ease;box-shadow:0 1px 2px rgba(0,0,0,0.04);margin-bottom:.5rem}petshop--organism-admin-scheduling-102009 .user-item:hover{background:#f9fafb;transform:translateY(-1px);border-color:#d1d5db}petshop--organism-admin-scheduling-102009 .user-item:active{background:#f3f4f6;transform:scale(.99)}petshop--organism-admin-scheduling-102009 .icon{display:flex;align-items:center;justify-content:center;font-size:1.4em;background:#f3f4f6;color:#4b5563;border-radius:6px;width:36px;height:36px}`);
    this.loading = false;
    this.users = [];
    this.pets = [];
    this.services = [];
  ...
  render() {
    return Z`<div class="scheduling-container">
  ...
  renderSelectUser() {
    return Z`
        ...
        <div class="form-group">
            ...
            <input type="text" id="user-search" placeholder="Digite para pesquisar" @input=${(e6) => {
      this.searchText = e6.target.value;
    }} @keydown=${(e6) => {
      if (e6.key === "Enter")
        setState2("ui.petshop.admin.organismAdminScheduling.action", "search");
    }}>
        </div>
        ...
        ${this.labelError ? Z`<span class="error-message">${this.labelError}</span>` : ""}
  ...
  renderItemList(user) {
    return Z`
        <li class="user-item" @click=${() => this.handleContinue(user)}>
            ...
            <span>${user.details.registrationData.name}</span>
        </li>`;
  }
  renderAddScheduling() {
    return Z`
            ...
            <div class="form-group">
                ...
                <input readonly type="text" value="${(this.userSelected?.details.registrationData).name}"}
            </div>
            <div class="form-group">
                <label for="pet-select">Selecionar Pet</label>
                <select id="pet-select" @change=${(e6) => {
      this.petSelected = this.pets[+e6.target.value];
    }}>
                    <option value="">Escolha um pet</option>
                    ${this.pets.map((pet, index) => Z`<option value="${index}">${pet.details.registrationData.name}</option>`)}
                </select>
            </div>
            <div class="form-group">
                <label for="date-select">Data</label>
                <input type="datetime-local" id="date-select" value="2025-10-15" @input=${(e6) => {
      this.date = e6.target.value;
    }}
            </div>
            <div class="form-group">
                <label for="service-select">Serviço</label>
                <select id="service-select" @change=${(e6) => {
      this.serviceSelected = this.services[+e6.target.value];
    }}>
                    <option value="">Escolha um serviço</option>
                    ${this.services.map((pet, index) => Z`<option value="${index}">${pet.details.registrationData.name}</option>`)}
                </select>
            </div>
            <div class="form-actions">
                ...
                <button class="btn btn-save" @click=${this.handleSave} ?disabled=${this.loading}>
                    Agendar
                    ${this.loading ? Z`<span class="loading"></span>` : Z``}
                </button>
...
`;
  ...
  async getMyPets(user) {
    ...
    if (user.details.relationships) {
      user.details.relationships.forEach((r4) => {
  ...
  async handleSave() {
    ...
    if (!this.petSelected) {
      this.labelError = "Pet obrigat\xF3rio.";
      return;
    }
    if (!this.serviceSelected) {
      this.labelError = "Selecione um servi\xE7o";
      return;
    }
    if (!this.date) {
    ...
    const now = new Date();
    if (selectedDate < now) {
      this.labelError = "A data e hora n\xE3o podem ser anteriores \xE0 atual.";
      return;
    }
    this.loading = true;
    let phoneClient = "";
    ...
    const params = {
      details: {
        ...
        tutor: {
          ...
          name: this.userSelected.details.registrationData.name,
          phone: phoneClient
        },
        pet: {
          petMdmId: this.petSelected.id || 0,
          name: this.petSelected.details.registrationData.name,
          species: this.petSelected.details.registrationData.species,
          breed: this.petSelected.details.registrationData.breed,
          allergies: []
        },
        service: {
          serviceMdmId: this.serviceSelected.id || 0,
          name: this.serviceSelected.details.registrationData.name,
          serviceCode: this.serviceSelected.details.registrationData.serviceCode
        }
      }
    };
    const req = {
  ...
  async handleSearch() {
    ...
    this.users = mdm.data.filter((item) => item.details.type === MdmType.PessoaFisica);
    this.loading = false;
    setState2("ui.petshop.admin.organismAdminScheduling.action", "");
  }
};
__decorate6([
  U2()
...
organismAdminScheduling = __decorate6([
  e2("petshop--organism-admin-scheduling-102009")
