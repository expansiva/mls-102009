/// <mls shortName="pageAdminEditScheduling" project="102009" enhancement="_blank" folder="wwwroot/petshop" />
// virtual:lit
var t = globalThis;
...
// virtual:/_102009_petshop/pageAdminEditScheduling
...
var PageAdminEditScheduling102009 = class PageAdminEditScheduling1020092 extends CollabPageElement {
  constructor() {
    super();
    if (this.loadStyle)
      this.loadStyle(`petshop--page-admin-edit-scheduling-102009 {
  display: flex;
}
petshop--page-admin-edit-scheduling-102009 main {
  height: 100vh;
  overflow: auto;
  width: 100%;
}
@media (max-width: 768px) {
  petshop--page-admin-edit-scheduling-102009 {
...
PageAdminEditScheduling102009 = __decorate3([
  e2("petshop--page-admin-edit-scheduling-102009")
], PageAdminEditScheduling102009);
...
// virtual:/_102009_petshop/organismEditScheduling
...
var organismEditScheduling = class organismEditScheduling2 extends IcaOrganismBase {
  constructor() {
    super(...arguments);
    if (this.loadStyle)
  ...
  async firstUpdated(_changedProperties) {
    super.firstUpdated(_changedProperties);
    const data = getState2("ui.petshop.admin.scheduling.selected");
  ...
  render() {
    ...
    return Z`
        ...
        <ul class="scheduling-summary">
            ...
            ${this.schedulingData?.details.status === SchedulingStatus.PENDING ? Z`
                    <li class="li-edit">
                        ...
                        <div class="form-group">
                            <select id="employees"
                                ...
                                @change=${(e5) => {
      this.employerSelected = this.employees[+e5.target.value];
    }}
                                
                            >
                                <option value="">Selecione o funcionário</option>
                                ${this.employees.map((item, index) => {
      const name = item.details.type === MdmType.PessoaFisica ? item.details.registrationData.name : item.details.registrationData.fantasyName;
      return Z`<option value=${index}> ${name}</option>`;
    })}
                            </select>
                        </div>
                    </li>
        ...
        <div class="form-actions">
            ${this.schedulingData?.details.status === SchedulingStatus.PENDING ? Z`
                    <button class="btn btn-delete" @click=${this.handleClickCancel} ?disabled=${this.loadingCancel || this.loadingConfirm} ?hidden=${!isPending}>
                        Recusar Serviço
                        ${this.loadingCancel ? Z`<span class="loading"></span>` : Z``}
                </button>` : ""}
            ${this.schedulingData?.details.status === SchedulingStatus.PENDING ? Z`<button class="btn btn-save" @click=${this.handleClickGenerateOS} ?disabled=${this.loadingConfirm || this.loadingCancel} ?hidden=${!isPending}>
                    Aprovar Serviço
                    ${this.loadingConfirm ? Z`<span class="loading"></span>` : Z``}
                </button>` : ""}
        </div>
  ...
  renderOrderService() {
    ...
    return Z`
            ...
            ${this.schedulingData?.details.serviceOrderId ? Z`
                <div class="section-card">
                    ...
                    <ul class="scheduling-summary">
                        ...
                        <li>
                            Valor:
                            <span>
                                ${this.orderData?.details.totalAmount != null ? this.orderData.details.totalAmount.toLocaleString("pt-BR", {
      style: "currency",
      currency: "BRL"
    }) : ""}
                            </span>
                        ...
                        <li>Status: <span>${statusText}</span></li>
                        <li><a href="/pageAdminOrderEdit" @click=${() => {
      setState2("ui.petshop.admin.order.selected", this.orderData);
    }}>Ver detalhes</a></li>
  ...
  async getService(id) {
    ...
    if (!response.ok) {
      this.labelError = response.error || "N\xE3o encontrado informa\xE7\xF5es do servi\xE7o";
      return;
    }
  ...
  async getOrderData(id) {
    ...
    if (!response.ok) {
      this.labelError = response.error || "N\xE3o encontrado informa\xE7\xF5es da ordem de servi\xE7o";
      return;
    }
  ...
  async getEmployers() {
    const ids = [];
    if (!this.service) {
      this.labelError = "N\xE3o encontrado informa\xE7\xF5es do servi\xE7o";
      return;
    }
    if (this.service.details.relationships) {
      this.service.details.relationships.forEach((r4) => {
  ...
  async handleClickCancel() {
    ...
    if (this.schedulingData.details.status !== SchedulingStatus.PENDING) {
      this.labelError = "A\xE7\xE3o permitida apenas para agendamentos pendentes";
      return;
    }
  ...
  async handleClickGenerateOS() {
    ...
    if (agendamentoDate < agora) {
      this.labelError = "N\xE3o \xE9 permitido gerar OS para agendamentos com data/hora passada!";
      return;
    }
    if (!this.employerSelected || !this.employerSelected.id) {
      this.labelError = "Selecionar funcion\xE1rio";
      return;
    }
    this.loadingConfirm = true;
    const nameEmployer = this.employerSelected.details.type === MdmType.PessoaFisica ? this.employerSelected.details.registrationData.name : this.employerSelected.details.registrationData.fantasyName;
    const data = {
      details: {
        schedulingId: this.schedulingData.id,
        status: ServiceOrderStatus.WAITING,
        totalAmount: (this.service?.details).serviceData?.priceRegular || 0,
        ...
        serviceProvided: {
          ...
          priceCharged: (this.service?.details).serviceData?.priceRegular || 0,
          name: (this.service?.details.registrationData).name || ""
        },
        employee: {
  ...
  async updateScheduling(id) {
    ...
    if (!id) {
      this.labelError = "Nenhum ordem de servi\xE7o encontrada!";
      return false;
    }
...
organismEditScheduling = __decorate6([
  e2("petshop--organism-edit-scheduling-102009")
